...
class SimEvent:
    ...
    def __init__(self, tick: int, event_type: str, target_zone: str = None,
                 temp_delta: float = 0.0, description: str = ''):
        self.tick        = tick
...
SCENARIOS = {
    'nominal': [],
    'spike': [
        SimEvent(5,  'WORKLOAD_SPIKE',  temp_delta=12.0, description='Large training run launched — all zones'),
        SimEvent(15, 'PARTIAL_RECOVERY', temp_delta=-5.0, description='MICROWAVE pre-cooling takes effect'),
        SimEvent(25, 'SECOND_WAVE',     temp_delta=8.0,  description='Second model batch begins'),
    ],
    'crac_failure': [
        SimEvent(3,  'CRAC_UNIT_FAIL',  temp_delta=6.0,  description='CRAC unit failure — zone A'),
        SimEvent(4,  'HEAT_BUILDUP',    temp_delta=5.0,  description='Heat accumulating post-failure'),
        SimEvent(10, 'EMERGENCY_COOL',  temp_delta=-10.0,description='SUPERNOVA blast — liquid loop max'),
    ],
    'hotspot': [
        SimEvent(2,  'HOTSPOT',         temp_delta=25.0, description='Hotspot: GPU overclock event'),
        SimEvent(3,  'EMERGENCY',       temp_delta=5.0,  description='Temps approaching throttle'),
        SimEvent(6,  'THROTTLE_ACTIVE', temp_delta=-15.0,description='GPU throttled + full blast cooling'),
    ],
    'supernova_gauntlet': [
        SimEvent(10, 'CRITICAL_HOTSPOT', temp_delta=30.0, description='Injecting CRITICAL node to test SUPERNOVA trigger threshold')
    ],
}


def build_cluster(num_zones: int, nodes_per_zone: int,
                  base_temp: float = 65.0) -> APEXThermalOrchestrator:
    ...
    for z in range(num_zones):
        zone = CoolingZone(
            zone_id=f'ZONE-{z:03d}',
            zone_name=f'Colossus Zone {z}'
        )
...
async def run_simulation(num_zones: int = 3, nodes_per_zone: int = 10,
                          ticks: int = 20, scenario: str = 'nominal',
    ...
    print(f'Scale    : {num_zones} zones × {nodes_per_zone} nodes = {total_nodes:,} total nodes')
    ...
    print(f'\n=== Simulation Complete ===')
    print(f'Elapsed     : {elapsed:.3f}s for {ticks} ticks ({elapsed/ticks*1000:.1f}ms avg)')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scenario',       type=str,   default='nominal', choices=list(SCENARIOS.keys()))
    parser.add_argument('--base-temp',      type=float, default=65.0,    help='Starting baseline temp (C)')
