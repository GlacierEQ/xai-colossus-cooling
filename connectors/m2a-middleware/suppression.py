...
class SuppressionEngine:
    ...
    def apply(
        self, eligible: List[NodeEntry], request: Dict[str, Any]
        ...
        nodes.sort(
            key=lambda n: (
                -(
                    n.priority
                    - (DEGRADED_PENALTY if n.status == "degraded" else 0)
                    - n.effective_load() * 0.1
                )
            )
        )
        ...
        logger.info(
            f"[Suppression] selected={len(selected)} suppressed={suppressed} cap={max_r}"
        )
