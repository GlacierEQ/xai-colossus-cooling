...
class BundleAggregator:
    def __init__(
        self, registry: NodeRegistry, aspen_grove_client=None, influx_client=None
    ):
    ...
    async def collect(
        ...
        tasks = {
            n.node_id: asyncio.create_task(self._dispatch(n, request, dispatch_fn))
        ...
        status = self._status(
            ranked, min_responses, len(selected_nodes), timeout_ms, elapsed_ms
        )
        bundle = self._build(
            request["request_id"],
            ranked,
            status,
            len(selected_nodes) + suppressed_count,
            suppressed_count,
            len(ranked),
            elapsed_ms,
            len(ranked) >= min_responses,
        )
    ...
    async def _dispatch(
        self, node: NodeEntry, request: dict, dispatch_fn
    ) -> Optional[dict]:
        ...
        try:
            ...
            return {
                ...
                "rank_score": 0.0,
                "confidence": payload.get("confidence", 1.0) if payload else 0.0,
                "payload": payload or {},
                "error": None,
            }
    ...
    def _rank(self, responses: List[dict]) -> List[dict]:
        ...
        for r in responses:
            r["rank_score"] = round(
                0.7 * r["confidence"] + 0.3 * (1.0 - r["latency_ms"] / max_lat), 4
            )
    ...
    def _status(self, responses, min_r, total_sel, timeout_ms, elapsed_ms) -> str:
        ...
        if not responses:
            return "timeout"
        if len(responses) >= min_r:
            return "complete" if elapsed_ms < timeout_ms * 0.95 else "partial"
        return "partial"
    ...
    async def _emit_ag(self, bundle: dict, request: dict):
        if not self.aspen_grove:
            return
        try:
            eid = await self.aspen_grove.log_event(
                {
                    "event_type": "m2a_bundle_closed",
    ...
    async def _emit_influx(self, bundle: dict):
        ...
        try:
            await self.influx.write(
                measurement="m2a_bundles",
                tags={"status": bundle["status"]},
                fields={
                    "total_eligible": bundle["metadata"]["total_eligible"],
