...
class SensorMap:
    ...
    def _build(self):
        for zone_id in self.ZONES:
            racks = [f"{zone_id}-rack-{r:02d}" for r in range(1, self.RACKS_PER_ZONE + 1)]
            cdus  = [f"{zone_id}-cdu-{c:02d}"  for c in range(1, self.CDU_PER_ZONE + 1)]
            self._zones[zone_id] = ZoneTopology(zone_id=zone_id, cdu_ids=cdus, rack_ids=racks,
                                                sensor_count=142, thermal_load_mw_design=50.0)
            ids = []
            for stype, count, model, acc, unit, hz in self.SENSOR_TYPES:
                for i in range(count):
                    sid = f"{zone_id}-{stype}-{i:03d}"
                    self._sensors[sid] = SensorEntry(
                        sensor_id=sid, zone_id=zone_id, rack_id=racks[i % len(racks)],
                        cdu_id=cdus[i % len(cdus)], sensor_type=stype,
                        location="inlet" if i % 2 == 0 else "outlet",
                        accuracy_pct=acc, sample_rate_hz=hz, hardware_model=model, commissioned="2026-01-15")
                    ids.append(sid)
            self._zone_index[zone_id] = ids

    def zone_sensors(self, zone_id: str) -> List[SensorEntry]: return [self._sensors[s] for s in self._zone_index.get(zone_id, [])]
    def get(self, sensor_id: str) -> Optional[SensorEntry]: return self._sensors.get(sensor_id)
    def all_zones(self) -> List[str]: return list(self._zones.keys())
    def zone_topology(self, zone_id: str) -> Optional[ZoneTopology]: return self._zones.get(zone_id)
    def total_sensor_count(self) -> int: return len(self._sensors)
    def summary(self) -> dict:
        return {"zones": len(self._zones), "total_sensors": len(self._sensors),
                "by_type": {st: sum(1 for s in self._sensors.values() if s.sensor_type == st) for st, *_ in self.SENSOR_TYPES}}
