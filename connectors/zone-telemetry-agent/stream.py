...
class KafkaStreamProducer:
    def __init__(self, bootstrap_servers: str = "colossus-kafka-01:9092",
                 client_id: str = "colossus-telemetry-producer",
                 batch_size: int = 500, linger_ms: int = 5):
    ...
    async def connect(self):
        try:
            ...
            self._producer = AIOKafkaProducer(
                ...
                compression_type="lz4", batch_size=self.batch_size * 1024,
                linger_ms=self.linger_ms, acks="all", enable_idempotence=True)
    ...
    async def publish_batch(self, topic: str, records: List[Dict[str, Any]]):
        envelope = {"batch_id": str(uuid.uuid4()), "topic": topic, "count": len(records),
                    "produced_at": datetime.now(timezone.utc).isoformat(), "records": records}
        if self._producer:
            try:
    ...
    def stats(self) -> dict:
        return {**self._stats, "throughput_per_sec": self.throughput_per_sec(), "target_per_sec": THROUGHPUT_TARGET_PER_SEC}
