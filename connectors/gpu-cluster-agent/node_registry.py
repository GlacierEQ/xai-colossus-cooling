...
class NVL72Node:
    ...
    def __post_init__(self):
        if not self.gpus:
            self.gpus = [
                GPUState(gpu_id=f"{self.node_id}-gpu-{i:02d}", slot=i, model=self.gpu_model)
                for i in range(GPUS_PER_NODE)
            ]

    def update_thermals(self):
        if not self.gpus: return
        self.mean_die_temp_c = round(sum(g.die_temp_c for g in self.gpus) / len(self.gpus), 2)
        ...
        if self.alert_gpus > 0:
            self.status = "critical" if self.max_die_temp_c >= CRITICAL_TEMP_C else "throttling"
...
class NVL72NodeRegistry:
    ...
    def zone_nodes(self, zone_id: str) -> List[NVL72Node]:
        with self._lock:
            return [self._nodes[nid] for nid in self._zone_index.get(zone_id, []) if nid in self._nodes]
    ...
    def cluster_snapshot(self) -> dict:
        with self._lock:
            ...
            return {"total_nodes": len(nodes), "total_gpus": total_gpus,
                    "total_power_kw": total_power, "throttled_gpus": throttled,
                    "alert_gpus": alerts, "mean_die_temp_c": mean_temp,
                    "max_die_temp_c": max_temp, "healthy_nodes": sum(1 for n in nodes if n.status == "healthy")}
    ...
    def build_zone(self, zone_id: str, gpu_model: str = "H200"):
        ...
        for row in range(1, 25):  # 24 rows
            ...
            for rack in range(1, 84):  # ~83 racks per row to hit 1984 nodes
                ...
                node = NVL72Node(node_id=node_id, zone_id=zone_id, rack_id=rack_id,
                                 row_id=row_id, gpu_model=gpu_model,
