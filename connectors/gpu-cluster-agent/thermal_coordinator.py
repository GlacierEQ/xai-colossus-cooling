...
from .node_registry import NVL72NodeRegistry, NVL72Node, THROTTLE_TEMP_C
...
class ThermalThrottleCoordinator:
    ...
    async def _handle_throttle(self, node: NVL72Node):
        ...
        level = next(
            (
                name
                for thresh, _, name in THROTTLE_LEVELS
                if node.max_die_temp_c >= thresh
            ),
            "warning",
        )
        if level == "emergency_shutdown":
            ...
            logger.critical(
                f"[Thermal] EMERGENCY SHUTDOWN {node.node_id} max_temp={node.max_die_temp_c}C"
            )
        else:
            logger.warning(
                f"[Thermal] THROTTLE {node.node_id} level={level} max={node.max_die_temp_c}C throttled={node.throttled_gpus}"
            )
