...
from .node_registry import NVL72NodeRegistry, ZONES, GPU_MODELS
...
class GPUClusterAgent:
    ...
    async def initialize(self):
        ...
        logger.info(f"[GPUAgent] Ready — {total} NVL72 nodes / {total * 72:,} GPUs registered")
        ...
        if self.influx:
            try:
                await self.influx.write_reading(
                    ...
                    tags={"scope": "cluster"},
                    fields={"total_nodes": snapshot["total_nodes"],
    ...
    def stats(self) -> dict:
        return {**self._stats, "registry": self.registry.cluster_snapshot(),
