...
class ROPlantController:
    def __init__(self, alert_callback: Optional[Callable] = None,
                 influx_sink=None, kafka_callback: Optional[Callable] = None):
        ...
        self._membrane_health: List[MembraneHealth] = [
            MembraneHealth(stage_id=i, element_count=MEMBRANES_PER_STAGE,
                           diff_pressure_bar=0.3, normalized_flux=1.0,
                           salt_rejection_pct=99.5, last_cip=datetime.now(timezone.utc).isoformat(),
                           health_score=1.0)
            for i in range(1, MEMBRANE_STAGES + 1)
        ]
        self._history: List[ROReading] = []
        self._stats = {"readings": 0, "cip_cycles": 0, "tds_violations": 0, "warnings": 0}
    ...
    async def _read(self) -> ROReading:
        import random
        feed   = round(RATED_OUTPUT_LPM / RECOVERY_RATE_TARGET * random.uniform(0.98, 1.02), 2)
        ...
        product_tds = round(feed_tds * random.uniform(0.02, 0.06), 2)  # 94-98% rejection
        ...
        if product_tds > TDS_PRODUCT_MAX_PPM:
            status = "warning"; self._stats["tds_violations"] += 1
        if diff_p >= DIFF_P_CRITICAL_BAR:
            status = "cip_required"
        elif diff_p >= DIFF_P_WARNING_BAR:
            status = "warning"; self._stats["warnings"] += 1
        return ROReading(
            timestamp=datetime.now(timezone.utc).isoformat(),
            feed_flow_lpm=feed, permeate_flow_lpm=permeate, reject_flow_lpm=reject,
            recovery_rate=recovery, feed_tds_ppm=feed_tds, product_tds_ppm=product_tds,
            rejection_pct=rejection, feed_pressure_bar=round(random.uniform(8.0, 10.0), 2),
            diff_pressure_bar=diff_p, temp_c=round(random.uniform(17.5, 19.0), 2),
            status=status, cip_active=self._cip_active
        )
    ...
    async def _trigger_cip(self, r: ROReading):
        ...
        if self.kafka_cb:
            asyncio.create_task(self.kafka_cb("colossus.cooling.events", {
                "event_id": str(uuid.uuid4()), "event_type": "ro_cip_start",
                "timestamp": r.timestamp, "diff_pressure_bar": r.diff_pressure_bar}))
    ...
    async def _emit(self, r: ROReading):
        ...
        try:
            await self.influx.write_reading(
                ...
                fields={"permeate_lpm": r.permeate_flow_lpm, "product_tds_ppm": r.product_tds_ppm,
                        "rejection_pct": r.rejection_pct, "diff_pressure_bar": r.diff_pressure_bar,
                        "recovery_rate": r.recovery_rate, "cip_active": int(r.cip_active)},
                ts=r.timestamp)
        except Exception as e:
            logger.warning(f"[RO] InfluxDB emit failed: {e}")

    def membrane_health_summary(self) -> list:
        return [{"stage": m.stage_id, "health": m.health_score, "diff_p": m.diff_pressure_bar} for m in self._membrane_health]
