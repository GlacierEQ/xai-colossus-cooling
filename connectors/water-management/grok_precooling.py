...
class GrokPreCoolingEngine:
    ...
    def __init__(self, api_key: str = "",
        ...
        self._ws = None
        self._stats = {"forecasts": 0, "grok_hits": 0, "fallback_hits": 0, "prestages_triggered": 0}
    ...
    def update_telemetry(self, mean_temp_c: float, it_load_kw: float):
        ...
        if len(self._thermal_history) > 1800: self._thermal_history.pop(0)  # 30min @ 1s
        if len(self._load_history) > 1800: self._load_history.pop(0)
    ...
    async def forecast_15min(self) -> Dict[str, Any]:
        ...
        if forecast.pre_stage_valves:
            ...
            logger.info(f"[Grok] Pre-stage triggered: +{forecast.valve_prestage_pct}% "
    ...
    def _statistical_forecast(self) -> ThermalForecast:
        ...
        if len(self._thermal_history) < 10:
            return ThermalForecast(forecast_id=str(uuid.uuid4()),
                                   generated_at=datetime.now(timezone.utc).isoformat(),
        ...
        prestage = conf >= CONFIDENCE_THRESHOLD and predicted_delta >= THERMAL_SPIKE_DELTA_C
        return ThermalForecast(
            forecast_id=str(uuid.uuid4()),
            generated_at=datetime.now(timezone.utc).isoformat(),
