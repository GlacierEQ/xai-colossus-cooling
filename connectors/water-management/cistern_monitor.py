...
class CisternMonitor:
    def __init__(
        self,
        initial_volume_l: float = CISTERN_CAPACITY_L,
    ...
    async def _read(self) -> CisternReading:
        ...
        self._volume = max(
            0.0, min(CISTERN_CAPACITY_L, self._volume + (fill - drain) * 0.5)
        )
        ...
        try:
            await self.influx.write_reading(
                ...
                fields={
                    ...
                    "drain_rate_lpm": r.drain_rate_lpm,
                    "fill_rate_lpm": r.fill_rate_lpm,
                    "leak_detected": int(r.leak_detected),
                },
                ts=r.timestamp,
            )
    ...
    def trend(self) -> dict:
        ...
        return {
            "current_volume_l": self._history[-1].volume_litres,
            "current_level_pct": self._history[-1].level_pct,
            "autonomy_hours": self._history[-1].autonomy_hours,
            "vol_trend_lpm": round(
                statistics.mean([r.net_rate_lpm for r in self._history]), 3
            ),
            "capacity_l": CISTERN_CAPACITY_L,
            "status": self._history[-1].level_status,
        }
