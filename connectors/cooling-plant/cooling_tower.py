...
class CoolingTowerController:
    def __init__(self, kafka_callback: Optional[Callable] = None):
        ...
        self.towers = {f"CT-{i+1:02d}": CoolingTower(f"CT-{i+1:02d}") for i in range(NUM_TOWERS)}
    ...
    async def legionella_check(self):
        ...
        for t in self.towers.values():
            if t.legionella_risk_score > 7.0:
                logger.critical("LEGIONELLA RISK CT %s — initiating thermal shock", t.unit_id)
    ...
    async def zld_blowdown(self, tower_id: str):
        ...
        await self._emit("zld_blowdown_start", {"unit": tower_id,
                                                 "coc": t.cycles_of_concentration})
    ...
    def snapshot(self):
        return {
            ...
            "avg_cws_temp_c": round(
                sum(t.condenser_water_supply_c for t in self.towers.values()) / NUM_TOWERS, 2),
            "stats": self.stats,
        }

    async def _emit(self, event: str, extra: dict = {}):
        if self.kafka_cb:
            await self.kafka_cb("colossus.cooling.tower", {
