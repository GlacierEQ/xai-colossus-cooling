...
LOAD_SHED_TIERS: Dict[int, dict] = {
    ...
    5: {"label": "Lighting / HVAC Non-Critical","shed_mw": 15,   "trigger_deficit_mw": 40},
}
...
class ATSSTSController:
    ...
    async def ats_transfer(self, to_source: PowerSource) -> TransferEvent:
        ...
        await asyncio.sleep(0.080)  # Simulate 80 ms mechanical close + synchronise check
        duration_ms = (time.perf_counter() - t0) * 1000
        success = duration_ms <= self.ATS_TARGET_MS
        self.active_source = to_source
        self.stats["ats_transfers"] += 1
        ev = TransferEvent(
            event_id=str(uuid.uuid4()), from_source=self.active_source.value,
            to_source=to_source.value, transfer_type="ATS",
            duration_ms=round(duration_ms, 2), success=success,
            timestamp=datetime.now(timezone.utc).isoformat(),
        )
        self.transfer_log.append(ev)
        await self._emit("ats_transfer", {"duration_ms": ev.duration_ms, "success": success,
                                          "to_source": to_source.value})
        if not success:
            logger.error("ATS transfer EXCEEDED target: %.1f ms (target %s ms)",
                         duration_ms, self.ATS_TARGET_MS)
        return ev
    ...
    async def sts_transfer(self, to_source: PowerSource) -> TransferEvent:
        ...
        ev = TransferEvent(
            event_id=str(uuid.uuid4()), from_source=self.active_source.value,
            to_source=to_source.value, transfer_type="STS",
            duration_ms=round(duration_ms, 2), success=success,
            timestamp=datetime.now(timezone.utc).isoformat(),
        )
        self.transfer_log.append(ev)
        await self._emit("sts_transfer", {"duration_ms": ev.duration_ms, "success": success,
                                          "to_source": to_source.value})
        return ev
    ...
    async def shed_load(self, deficit_mw: float):
        ...
        for tier, spec in sorted(LOAD_SHED_TIERS.items(), reverse=True):
            ...
            await self._emit("load_shed_tier", {"tier": tier, "label": spec["label"],
                                                "shed_mw": spec["shed_mw"]})
            logger.warning("LOAD SHED Tier %d — %s — %.0f MW removed",
                           tier, spec["label"], spec["shed_mw"])
        return shed_total
    ...
    def transfer_report(self) -> dict:
        return {
            ...
            "ats_within_target": sum(1 for e in self.transfer_log
                                     if e.transfer_type == "ATS" and e.success),
            "sts_within_target": sum(1 for e in self.transfer_log
                                     if e.transfer_type == "STS" and e.success),
    ...
    async def _emit(self, event: str, extra: dict = {}):
        if self.kafka_cb:
            await self.kafka_cb("colossus.power.transfer", {
