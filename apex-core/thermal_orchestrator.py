    ...
    async def _sweep_zone(self, zone: CoolingZone) -> dict:
        ...
        if budget_warn:
            self.logger.warning(
                ...
                zone.zone_id, zone_draw_kw, zone.thermal_budget_kw,
...
async def _dispatch_fusion(piston_map: dict, fusion_name: str, context: dict) -> dict:
    ...
    if not required:
        logger.warning('Fusion mode %s has no requires list — nothing dispatched', fusion_name)
    ...
    for pname in required:
        ...
        if piston is None:
            ...
            results[pname] = await piston.activate({**context, 'trigger': f'FUSION_{fusion_name}'})
...
class APEXThermalOrchestrator:
                ...
                if zone.zone_id in cmap:
                    ...
                    if abs(zone.conductivity_factor - prev) > 0.02:
                        self.logger.info(
                            ...
                            zone.zone_id, prev, zone.conductivity_factor,
                        )
        except Exception as e:
    ...
    async def tick_cycle(self, power_snapshot: dict = None):
        ...
        refresh_interval = max(int(self.tick_cfg.get('connector_refresh_every_n_ticks', 10)), 1)

        # FIX: read fabric diagnostic cadence from manifest (default 20 ticks).
        fabric_interval  = max(int(self.tick_cfg.get('fabric_diagnostic_every_n_ticks', 20)), 1)
