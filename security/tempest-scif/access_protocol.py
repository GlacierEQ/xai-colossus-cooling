    ...
    async def request_inner(self) -> bool:
        async with self._lock:
            if not self._door_states["outer"]:
                logger.warning(
                    f"[ManTrap:{self.trap_id}] Inner denied — outer not open"
                )
                return False
            if self._door_states["inner"]:
                return True
...
class AccessControlSystem:
    ...
    async def process(self, req: AccessRequest) -> AccessResult:
        ...
        if not person or person["badge_id"] != req.badge_id:
            return await self._deny(
                req, "invalid_badge", alert=True, alert_type="BADGE_MISMATCH"
            )

        # Factor 2: active status
        if not person["active"]:
            return await self._deny(
                req, "revoked", alert=True, alert_type="REVOKED_ATTEMPT"
            )

        # Factor 3: biometric
        if person["biometric_hash"] != req.biometric_hash:
            return await self._deny(
                req, "biometric_fail", alert=True, alert_type="BIO_MISMATCH"
            )
    ...
    async def _grant(
        self, req: AccessRequest, clearance: ClearanceLevel
    ) -> AccessResult:
        result = AccessResult(
            request_id=req.request_id,
            granted=True,
            reason="all_factors_passed",
        ...
        logger.info(
            f"[Access] GRANTED {req.person_id} zone={req.target_zone.name} door={req.door_id}"
        )
        await self._emit(result)
        return result

    async def _deny(
        self, req: AccessRequest, reason: str, alert: bool = False, alert_type: str = ""
    ) -> AccessResult:
        result = AccessResult(
            request_id=req.request_id,
            granted=False,
        ...
        logger.log(
            level, f"[Access] DENIED {req.person_id} reason={reason} alert={alert_type}"
        )
        await self._emit(result)
        return result
