...
class TamperEvidentAuditLogger:
    ...
    def __init__(
        self,
        secret_key: bytes = b"CHANGE-IN-PRODUCTION-USE-HSM",
    ...
    async def log(
        ...
        async with self._lock:
            ...
            entry.chain_hash = hmac.new(
                self._key, mac_input, hashlib.sha256
            ).hexdigest()
    ...
    async def verify_chain(self, entries: List[AuditEntry]) -> bool:
        ...
        for entry in entries:
            ...
            if not hmac.compare_digest(expected, entry.chain_hash):
                ...
                logger.critical(
                    f"[Audit] CHAIN BREAK at sequence={entry.sequence} entry={entry.entry_id}"
                )
                return False
    ...
    async def _emit_kafka(self, entry: AuditEntry):
        if not self._kafka_cb:
            return
        try:
            await self._kafka_cb(
                "colossus.security.audit",
                {
                    "entry_id": entry.entry_id,
                    "sequence": entry.sequence,
                    "event_type": entry.event_type,
