...
async def run(ci: bool = False, fail_on_critical: bool = True) -> dict:
    ...
    for zi, zid in enumerate(["ZONE-A", "ZONE-B"]):
        ...
        for ni in range(5):
            node = ThermalNode(
                node_id=f"{zid}-N{ni:02d}",
                rack_id=f"RACK-{zi:02d}",
                zone_id=zid,
                temp_celsius=60.0 + ni * 6.0,  # 60–84°C
                gpu_utilization=0.9,
                power_watts=700.0,
            )
    ...
    sn = MagicMock()
    sn.activate = AsyncMock(return_value={"piston": "SUPERNOVA", "actions": []})
    mw = MagicMock()
    mw.activate = AsyncMock(return_value={"piston": "MICROWAVE", "zones_swept": 2})
    orch.pistons = {"SUPERNOVA": sn, "MICROWAVE": mw}
    orch.run_fusion_mode = AsyncMock(
        return_value={"fusion": "UNKNOWN", "status": "UNKNOWN"}
    )
    ...
    if mw.activate.call_count < 2:
        failures.append(
            f"MICROWAVE called {mw.activate.call_count} times, expected >=2"
        )
    ...
    logger.info(
        "MCP_TICK_INTEGRATION: %s | %d requests in %.1f ms",
        status,
        len(results),
        elapsed_ms,
    )
