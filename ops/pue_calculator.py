...
class PUECalculator:
    def __init__(self, alert_callback: Optional[Callable] = None,
        ...
        self._stats = {"samples": 0, "pue_warnings": 0, "pue_criticals": 0,
                       "wci_warnings": 0, "wci_criticals": 0}

    def record(self, it_load_kw: float, total_facility_kw: float,
               cooling_kw: float = 0.0, lighting_kw: float = 0.0,
               ups_loss_kw: float = 0.0,
               water_consumed_litres: float = 0.0,
               measurement_period_hours: float = 1.0) -> PUESample:
        ...
        sample = PUESample(
            ...
            cooling_kw=cooling_kw, lighting_kw=lighting_kw, ups_loss_kw=ups_loss_kw,
            water_consumed_litres=water_consumed_litres,
            measurement_period_hours=measurement_period_hours,
            pue=pue, wci_ml_per_kwh=wci, pue_status=pue_status, wci_status=wci_status
        )
        ...
        if pue_status != "ok" or wci_status != "ok":
            ...
            logger.info(f"[PUE] pue={pue} wci={wci} IT={it_load_kw}kW total={total_facility_kw}kW")
    ...
    def trend(self) -> dict:
        ...
        return {
            ...
            "pue_vs_industry": round(((INDUSTRY_AVG_PUE - latest.pue) / INDUSTRY_AVG_PUE) * 100, 1),
