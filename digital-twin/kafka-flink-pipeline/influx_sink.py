...
RETENTION_POLICIES = [
    {
        ...
        "duration": "24h",
        "shard": "1h",
        "series": ["colossus_telemetry", "colossus_gpu_thermal"],
    },
    {
        "name": "warm_7d",
        "duration": "7d",
        "shard": "1d",
        "series": ["colossus_aggregated", "colossus_water_flow"],
    },
    {
        "name": "cool_90d",
        "duration": "90d",
        "shard": "7d",
        "series": ["colossus_alerts"],
    },
...
DOWNSAMPLE_TASKS = [
    {
        "name": "raw_to_1min",
        "from": "colossus_telemetry",
        "to": "colossus_aggregated",
        "every": "1m",
        "fn": "mean",
    },
    {
...
class InfluxV3Sink:
    def __init__(
        self,
        url: str = "http://colossus-influx-01:8086",
        token: str = "",
        org: str = "glaciereq",
        bucket: str = "colossus_telemetry",
    ):
    ...
    async def write_pue(
        self, it_load_kw: float, total_facility_kw: float, wci_ml_per_kwh: float = 0.0
    ):
        ...
        await self.write_reading(
            ...
            fields={
                "pue": pue,
                "pue_delta": round(pue - PUE_TARGET, 4),
                "it_load_kw": it_load_kw,
                "total_facility_kw": total_facility_kw,
                "wci_ml_per_kwh": wci_ml_per_kwh,
                "sla_met": int(pue <= PUE_TARGET),
            },
        )

    async def write_batch(self, records: List[Dict[str, Any]]):
        await asyncio.gather(
            *[self.write_reading(**r) for r in records], return_exceptions=True
        )
    ...
    def schema_contracts(self) -> dict:
        return {
            ...
            "downsample": DOWNSAMPLE_TASKS,
            "pue_target": PUE_TARGET,
            "wci_target": WCI_TARGET,
        }
