...
class AdaptivePID:
    ...
    def update(self, current_temp: float, target: float) -> float:
        ...
        effective_Kp = self.Kp * (
            2.0 if (error > self.prev_error and error > 5) else 1.0
        )
        ...
        output = (
            (effective_Kp * error) + (self.Ki * self.integral) + (self.Kd * derivative)
        )
...
class ColossusThermalIntelligence:
    def __init__(self):
        logging.basicConfig(
            level=logging.INFO, format="%(asctime)s - [HYPER-THERMAL] - %(message)s"
        )
    ...
    def execute_logic_step(self):
        ...
        if temp > PRE_CRITICAL_C:
            self.logger.warning(
                f"PRE-CRITICAL DETECTED ({temp:.2f}C). Bypassing PID for Full Flow Pulse."
            )
            flow = 1.0
        else:
        ...
        self.logger.info(
            f"[{self.reality}] Temp: {temp:.2f}C | Adaptive Flow: {flow * 100:.1f}%"
        )


if __name__ == "__main__":
    print("\033[1m\033[94m[COLOSSUS PRIME COMPLETION: THERMAL INTELLIGENCE]\033[0m")
