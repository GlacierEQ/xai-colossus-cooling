...
class RedfishSensorReading:
    ...
    def __init__(self, node_id: str, rack_id: str, zone_id: str,
                 inlet_temp_c: float, exhaust_temp_c: float,
                 gpu_temps_c: List[float], power_watts: float,
                 fan_rpm: int, source: str = 'redfish'):
...
class RedfishSensorInterface:
    ...
    def poll_node(self, node_id: str, rack_id: str, zone_id: str,
                  base_temp: float = 65.0) -> RedfishSensorReading:
    ...
    def _mock_reading(self, node_id: str, rack_id: str, zone_id: str,
                      base_temp: float = 65.0) -> RedfishSensorReading:
    ...
    def _redfish_reading(self, node_id: str, rack_id: str,
                         zone_id: str) -> Optional[RedfishSensorReading]:
        ...
        try:
            ...
            data = resp.json()
            temps = [t.get('ReadingCelsius', 65.0)
                     for t in data.get('Temperatures', [])]
            fans  = [f.get('Reading', 10000) for f in data.get('Fans', [])]
            return RedfishSensorReading(
                node_id=node_id, rack_id=rack_id, zone_id=zone_id,
    ...
    def poll_cluster(self, node_map: List[Dict]) -> List[RedfishSensorReading]:
        ...
        readings = []
        for n in node_map:
            readings.append(self.poll_node(
                n['node_id'], n['rack_id'], n['zone_id'],
