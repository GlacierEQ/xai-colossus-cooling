...
def _node(node_id: str, temp: float, zone_id: str = 'Z001') -> ThermalNode:
    return ThermalNode(
        ...
        temp_celsius=temp, gpu_utilization=0.85, power_watts=700.0,
    )
    ...
    for tick in range(1, num_ticks + 1):
        ...
        if spike_active:
            for node in orch.all_nodes:
                node.temp_celsius = min(node.temp_celsius + spike_temp_delta * 0.5, 105.0)
            spike_remaining -= 1
        ...
        peak_temp_seen = max(peak_temp_seen, max((n.temp_celsius for n in orch.all_nodes), default=0.0))
...
class TestNoCascadeFailure:
    ...
    async def test_isolation_resets_after_recovery(self):
        cascade = CascadePreventionProtocol(thresholds=DEFAULT_THRESHOLDS['cascade_limits'])
        for _ in range(2):
            await cascade.evaluate_zone('Z-A', {'delta_t_c': 20.0, 'power_surge_mw': 0.5})
...
class TestSHADOWAnomalyDetection:
    ...
    async def test_shadow_ema_converges(self):
        ...
        stable_temp = 70.0
        for _ in range(100):
            await piston.execute({'all_nodes': [_node('N1', stable_temp)], 'trigger': 'converge'})
