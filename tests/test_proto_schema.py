...
class GPUCoolingTelemetrySim:
    def __init__(
        self, cluster_id: int, total_gpus: int, flow_rate_lpm: float, pue_ratio: float
    ):
...
class TestProtobufTelemetrySchema(unittest.TestCase):
    def test_schema_fields(self):
        telem = GPUCoolingTelemetrySim(
            cluster_id=1, total_gpus=100000, flow_rate_lpm=45000.0, pue_ratio=1.08
        )
