...
def node(
    ...
    return NodeEntry(
        ...
        node_type="telemetry",
        pillar=pillar,
        capabilities=caps or ["zone_telemetry"],
        domains=domains or ["cooling"],
...
def req(
    rtype="request_zone_snapshot", pillar="telemetry", caps=None, max_r=10, timeout=500
):
    return {
...
async def dispatch(n, r):
    await asyncio.sleep(0.01)
    return {"temp_c": 22.4, "confidence": 0.95}
...
class TestNodeRegistry:
    def test_register(self):
        reg = NodeRegistry()
        reg.register(node())
        assert reg.get("tz-01") is not None

    def test_heartbeat(self):
        reg = NodeRegistry()
        reg.register(node())
        t = reg.get("tz-01").last_heartbeat
        time.sleep(0.05)
        reg.heartbeat("tz-01", "healthy", 20.0)
        assert reg.get("tz-01").last_heartbeat > t

    def test_by_pillar(self):
        reg = NodeRegistry()
        reg.register(node("n1", pillar="telemetry"))
        reg.register(node("n2", pillar="analytics"))
        assert len(reg.by_pillar("telemetry")) == 1
        assert len(reg.by_pillar("all")) == 2


class TestRelevanceRouter:
    def test_cap_match(self):
        reg = NodeRegistry()
        reg.register(node(caps=["zone_telemetry"]))
        assert len(RelevanceRouter(reg).evaluate(req(caps=["zone_telemetry"]))) == 1

    def test_cap_mismatch(self):
        reg = NodeRegistry()
        reg.register(node(caps=["audit_log"]))
        assert len(RelevanceRouter(reg).evaluate(req(caps=["zone_telemetry"]))) == 0

    def test_latency_filter(self):
        reg = NodeRegistry()
        reg.register(node("slow", latency="batch"))
        reg.register(node("fast", latency="realtime"))
...
class TestSuppressionEngine:
    ...
    def test_overloaded_dropped(self):
        ...
        assert any(n.node_id == "ok" for n in sel)
        assert sup == 1
...
class TestIntegration:
    def test_full_broadcast(self):
        mw = M2AMiddleware()
        mw.register_node(node())
        b = asyncio.run(mw.broadcast(req(caps=["zone_telemetry"]), dispatch))
        assert b["status"] in ("complete", "partial")
        assert b["metadata"]["total_responded"] >= 1
    ...
    def test_emergency_sla(self):
        ...
        for i in range(5):
            mw.register_node(
                node(f"rt-{i}", pillar="runtime", caps=["emergency_response"])
            )
        b = asyncio.run(
            mw.broadcast(
                req(
                    rtype="emergency_broadcast",
                    pillar="runtime",
                    caps=["emergency_response"],
                    timeout=500,
                ),
                dispatch,
            )
        )
        assert b["metadata"]["sla_met"] is True
        assert b["metadata"]["elapsed_ms"] < 500
