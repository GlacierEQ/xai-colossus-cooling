...
def _make_orchestrator(zones=None, nodes=None):
    ...
    mw_piston = MagicMock()
    mw_piston.activate = AsyncMock(
        return_value={"piston": "MICROWAVE", "zones_swept": len(orch.zones)}
    )
...
async def test_zone_budget_override_happy_path():
    ...
    router.queue_request(
        _req("zone_budget_override", {"zone_id": "ZONE-A", "budget_kw": 200.0})
    )
    results = await router.process_tick(orch)
...
async def test_zone_budget_override_missing_zone():
    ...
    router.queue_request(
        _req("zone_budget_override", {"zone_id": "ZONE-X", "budget_kw": 50.0})
    )
    results = await router.process_tick(orch)
