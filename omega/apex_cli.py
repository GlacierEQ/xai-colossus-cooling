...
logging.basicConfig(level=logging.INFO, format="[APEX] %(levelname)s -- %(message)s")
...
def main():
  ...
  python apex_cli.py status""",
    ...
    bp.add_argument(
        "--all", action="store_true", help="Export all formats: DXF+PDF+SVG+PNG"
    )
    ...
    bp.add_argument(
        "--sheet", type=str, default="CCL-002", help="Sheet number (default: CCL-002)"
    )
    ...
    bp.add_argument(
        "--supply", type=float, default=16.0, help="Supply temp °C (default: 16.0)"
    )
    bp.add_argument(
        "--ret", type=float, default=26.0, help="Return temp °C (default: 26.0)"
    )
    ...
    sh = subparsers.add_parser(
        "sherlock", help="Start SHERLOCK-SUPERNOVA anomaly detection"
    )
