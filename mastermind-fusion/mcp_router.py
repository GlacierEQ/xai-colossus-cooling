...
class MCPRequest:
    def __init__(self, request_id: str = "", request_type: str = "", **kwargs: Any) -> None:
...
class MCPResponse:
    def __init__(self, request_id: str, request_type: str, status: str, data: Dict[str, Any]) -> None:
        self.request_id = request_id
        self.request_type = request_type
        self.status = status
        self.data = data
    ...
    async def dispatch(self, req_dict: Any) -> MCPResponse:
        ...
        if req.request_type == RequestType.ZONE_SNAPSHOT:
            ...
            return MCPResponse(
                ...
                data={'status': 'EMITTED', 'reason': 'Emergency broadcast dispatched', 'fanned_out': True}
            )
