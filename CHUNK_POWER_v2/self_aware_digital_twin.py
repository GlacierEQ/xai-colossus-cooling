...
class PINNLayer:
    ...
    def predict(self, measurement: ZoneMeasurement) -> ZonePrediction:
        ...
        heat_physics_kw = (
            measurement.flow_measured_kg_s
            * CP_WATER
            * (measurement.t_return_measured_c - measurement.t_supply_measured_c)
        ) / 1000.0
        ...
        heat_neural_kw = (
            measurement.flow_measured_kg_s
            * CP_WATER
            * (t_return_neural - measurement.t_supply_measured_c)
        ) / 1000.0

        # Step 3: compute PINN residual (physics consistency check)
        residual = abs(heat_neural_kw - heat_physics_kw) / max(
            abs(heat_physics_kw), 1e-6
        )

        flagged = residual > self.VIOLATION_THRESHOLD
        if flagged:
            logger.warning(
                f"PINN VIOLATION [{measurement.zone_id}]: residual={residual:.3f} > {self.VIOLATION_THRESHOLD}"
            )
    ...
    def _neural_predict(self, m: ZoneMeasurement) -> float:
        ...
        delta_t_expected = (m.gpu_power_measured_kw * 1000.0) / (
            m.flow_measured_kg_s * CP_WATER
        )
...
class SelfAwareDigitalTwin:
    ...
    def __init__(self):
        ...
        logger.info(
            "SelfAwareDigitalTwin: PINN layer active — continuous calibration enabled"
        )
    ...
    def divergence_check(self) -> list[str]:
        ...
        for zid, pred in self.predictions.items():
            ...
            if meas is None:
                continue
            heat_actual = (
                meas.flow_measured_kg_s
                * CP_WATER
                * (meas.t_return_measured_c - meas.t_supply_measured_c)
            ) / 1000.0
    ...
    def twin_health(self) -> dict:
        ...
        avg_confidence = sum(
            p.model_confidence for p in self.predictions.values()
...
if __name__ == "__main__":
    ...
    for i in range(1, 7):
        ...
        pred = twin.ingest(m)
        print(
            f"Zone R{i:02d}: predicted_return={pred.t_return_predicted_c:.1f}°C "
            f"residual={pred.pinn_residual:.4f} flagged={pred.flagged}"
        )
    health = twin.twin_health()
    print(
        f"Twin Health: {health['zones_tracked']} zones | confidence={health['avg_confidence']} | flags={len(health['flagged_zones'])}"
    )
